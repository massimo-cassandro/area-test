...
## Note
The file `led-chars.svg` is generated by the `build-led-board-svg-files.mjs` script (run `'node scripts/build-led-board-svg-files.mjs'`). It builds the SVG file starting from the ones produced by Figma (in `readme-files/chars`). This allows to easily design the singke chars in Figma, giving at the same time, the maximum control on the outputted code.
